...
def upsert_case(case: dict):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.execute(
            f"REPLACE INTO escalations ({','.join(data.keys())}) VALUES ({placeholders})", tuple(data.values())
        )
        conn.commit()
...
def boss_escalation_check():
    ...
    for _, row in df.iterrows():
        if (row["spoc_notify_count"] or 0) >= 2 and row["spoc_boss_email"]:
            ...
            if last and datetime.now() - datetime.fromisoformat(last) > timedelta(hours=24):
                ...
                if send_email(row["spoc_boss_email"], subj, body, row["id"]):
                    ...
                    row_dict["spoc_notify_count"] = (row["spoc_notify_count"] or 0) + 1
...
if df.empty:
    ...
    st.markdown(
        f"**Open:** {(df.status == 'Open').sum()} | **In Progress:** {(df.status == 'In Progress').sum()} | **Resolved:** {(df.status == 'Resolved').sum()}"
    )
    cols = st.columns(3)
                ...
                with st.expander(f"{row['id']} – {row['issue'][:55]}"):
                    ...
                    if st.button("Notify SPOC", key=f"notify_{row['id']}"):
                        if new_spoc:
                            ...
                            body = f"Dear SPOC,\n\nPlease review the escalation issue:\n\n{row['issue']}"
                            if send_email(new_spoc, subj, body, row["id"]):
                                row_dict = row.to_dict()
                                row_dict.update(
                                    {
                                        "spoc_notify_count": (row["spoc_notify_count"] or 0) + 1,
                                        "spoc_last_notified": datetime.now().isoformat(),
                                        "spoc_email": new_spoc,
                                        "spoc_boss_email": new_boss,
                                    }
                                )
                                upsert_case(row_dict)
                                st.success("SPOC notified via email.")
